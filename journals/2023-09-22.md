...
- ---
- #### Use "printf"
    - `printf "file '%s'\n" *.m4a | sort > list.txt` # For files
      ```
      file 'output01.m4a'
      file 'output02.m4a'
      ...
      ```
        - `$(<files.txt)` # For a file containing the filenames (Replace `*.m4a` with it)
- ***Notes***
    - `<files.txt` # File redirection (Open the file and send its content to whatever is reading from standard input)
        - `$(<files.txt)` is faster than `$(cat files.txt)` (Because it reads the file directly)
    - `$()` # Command substitution (Execute the command inside and replace it with the output)
    - `%s` # Insert a string here (Placeholder)
    - `\n` # Newline
    - Because it is better than `sed`.
        - No regex needed
- ***References***
    - Artificial intelligence
- ---
