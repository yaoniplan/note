- #### Use "tmux"
    ...
    - `C-b :` # Prompt for a command (Vim-like)
        ...
        - `move-window -s 40:24 -t 20:` # Move window to another session
        - `kill-session -t 15` # Kill session target 15
        - `kill-window -t 40:24` # Kill window `24` inside session `40` (<session>:<window>)
          ```
          (6) - 40: 2 windows   ← session (name 40)
          (7) ├─> - 1: bash-    ← window (index 1)
          (8) │   └─> 0: bash*  ← pane (index 0)
          (9) ├─> - 24: bash-
          ```
