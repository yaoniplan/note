- #### Use "git"
    - Get latest 1 commit hash
        - `git log -1 --format=%H` # Full commit hash (40 characters)
        - Because to review the latest commit message of the GitHub repository. (e.g. https://github.com/yaoniplan/note/commit/{SHA})
    - Stash (Like a scratchpad)
        - `git stash push -u -m "auto-stash-$timestamp"` # Save current changes to a new stash entry (`-u`: Include untracked files)
        - `git stash pop` # Apply and remove stashed changes from stash list
        - `git stash list` # List all stashes
        - `git stash show -p stash@{0}` # Show detailed changes stored in that stash
    ...
    - Use .gitignore to ignore every file and directory except the ones specified
        - `vi ~/.config/store/supermarket/.gitignore`
          ...
          !src/
          # Ignore all hidden directories (Located at any depth)
          /**/.*/
          ```
    ...
    - `git diff --cached` # Display the difference after using git add
        ...
        - ` -- backend/workers.js` # Use `--` to treat `backend/workers.js` as a file, not as a branch or tag name
...
- ***Notes***
    - Use a GitHub mirror when can't access the Internet
        - Warning: `*.github.io` is not supported.
    ...
    - Solve the problem
      ...
      Connection closed by 198.18.0.6 port 22
      fatal: Could not read from remote repository.
...
- ***References***
    ...
    - https://tisyang.github.io/post/2024-09-12-a-simple-config-of-sing-box/
