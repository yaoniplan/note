...
- [[linux]] [[虚拟机]] [[virtualbox]]使用  
  1. sudo pacman -S virtualbox virtualbox-host-modules-arch //安装 virtualbox，内核是 linux 的选择：virtualbox-host-modules-arch(提供 host )提供 host modules；内核是 linux 其他版的选择：详见参考资料)
  ...
  * **注意事项**
  1. enable efi //开启 efi (对操作系统有好处，在安装前的设置中)
  2. host + f //full the screen(host: right + ctrl，这是 hotkey)
  3. remove filename.iso //移除 iso 文件，解决再次进入 Live 系统问题(在安装后)
  * **参考资料**
