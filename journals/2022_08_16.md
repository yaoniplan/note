...
- [[linux]]基础命令  
  ...
  8. `vim hello` //文本编辑器 vim 打开(若没有，则创建)一个名叫 hello 的文件(移动光标：上下左右键；编辑：将光标移动到相应位置，输入 `i` (insert)；退出编辑模式：按 `esc` 键；保存：输入 `:` ，在 `:` 后面输入 `w` (write)，写入保存；退出：在 `:` 后面输入 `q` (quit)，退出文件；如果编辑了，还没保存且不想要了：在 `:` )