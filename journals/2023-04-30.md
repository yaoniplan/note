- #### Use "ffmpeg"
    ...
    - `ffmpeg -f concat -safe 0 -i fileList.txt -c copy output.wma` # Merge videos
        - `ls *.wma | sort | sed "s/^/file '/;s/$/'/" > fileList.txt` # Generate file list
          ```
          file 'output01.wma'
          file 'output02.wma'
          ...
        ...
        - `ffmpeg -f concat -safe 0 -i fileList.txt -c:v copy -c:a aac -b:a 128K _20250527135448-1050-1350.mp4` # Solve the problem
          ...
          ```
        - `ffmpeg -f concat -safe 0 -i fileList.txt -c:a libmp3lame -b:a 128k output.mp3` # Sovle the problem
          ```
          # Re-encode once (slower but perfect)
          # Remove timestamp issues completely
          # Fixes small gaps/inconsistencies
          # Produces a "proper" continues file
          [mp3 @ 0x55d244686b80] Application provided invalid, non monotonically increasing dts to muxer in stream 0: 2040011366339 >= 2040011366337
          ```
- ***Notes***
    - `-i` # Input
    - `-ss` # Specify starting
...
- ---
- #### Use "sed"
    - `sed "s/^/file '/;s/$/'/"` # Do this substitution, and then do this other substitution
        - `;` # Seperator (The logic is the same as in Shell command)
...
- ***Notes***
    - `-w` # Width of number
    - `-s` # Seperator of number
    - Because to add line numbers to the contents of a file.
- ***References***
    - `man nl` # `/separator` `/width`
    - Artificial intelligence
- ---
