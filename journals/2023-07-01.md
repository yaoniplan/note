...
- ***Notes***
    - Enable hotspot if your computer has a wireless network card (Wi-Fi adapter)
        - `ip address` # Check if there is a wireless interface (e.g. `wlan0`)
        ...
        - https://gist.github.com/narate/d3f001c97e1c981a59f94cd76f041140 (Try this command line on the new system next time)
        - Use "nm-connection-editor" if cli doesn't work
        - `nm-connection-editor`
          ...
          | SSID                 | yaoniplan                 |
          | Mode                 | Hotspot                   |
          | Device               | wlan0 (80:56:F2:44:E2:98) |
          | Wi-Fi Security       | WPA/WPA2/WPA3 Personal    |
          | Password             | yaoniplan                 |
          | IPv4 Settings Method | Shared to other computers |
          | IPv6 Settings Method | Shared to other computers |
        - `cat /proc/sys/net/ipv4/ip_forward` # If it returns `0`, enable it
        - `doas vim /etc/sysctl.d/99-ipforward.conf` # Enable this feature will allow your phone to connect to the internet
          ```
          net.ipv4.ip_forward=1
          ```
        - `doas sysctl -p /etc/sysctl.d/99-ipforward.conf` # Make changes effective permanently
        - https://redlib.perennialte.ch/r/linuxquestions/comments/1d67ovf/
        - `doas pacman --sync network-manager-applet` # Install dependencies
    - Solve the problem
      ```
